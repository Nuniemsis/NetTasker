...
def create_task_in_db(db: Session) -> Task:
    """Creates a new task in the database."""
    task_id = str(uuid4())
    start_date = datetime.now(tz=timezone.utc)
    new_task = Task(id=task_id, start_date=start_date)
    db.add(new_task)
    db.commit()
    db.refresh(new_task)
    return new_task
...
def count_tasks_by_status(db: Session, status: TaskStatus) -> int:
    ...
    return db.query(Task).filter(Task.status == status.value).count()

def get_paginated_tasks(db: Session, limit: int, offset: int) -> List[Task]:
